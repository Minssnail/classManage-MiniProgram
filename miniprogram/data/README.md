...
24 秋规则是「选修」），因此课程按 `ruleCode` 分版本存放。

规则的适用范围按**学生**而非班级判定：班级在「班级管理 → 专业规则」上设默认版本，
个别学生（补入班级、留级、转专业等入学年份不同的）在「学生管理 → 专业规则」上单独指定，
学生自身的设置优先于班级默认。成绩统计会逐人取各自的规则版本，不会整班共用一套。
