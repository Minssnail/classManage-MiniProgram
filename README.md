...
「待补考」包含两类，学校的《批量导入选课记录》模板对两者一视同仁：

| 类型 | 判定 | 说明 |
| --- | --- | --- |
| 重修 | 该门课考过，但历次成绩都没及格 | 显示已考次数、最近学期与最好成绩 |
| 补修 | 在本人适用的专业规则里属于统设必修课，但从未考过 | 显示教学计划建议的学期 |

只算重修会漏掉从未选修过的必修课，因此两类合并计算。综合实践课（毕业设计等）
由分部考核完成，不通过报考补考的方式选课，不计入补修。

补修按**每个学生各自适用的规则版本**判定：同一门课在 23 秋规则里是必修(分部)、
在 24 秋规则里是选修，因此换规则版本会改变补修门数。
